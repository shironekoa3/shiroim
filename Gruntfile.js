...
module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        concat: { //合并代码
            ...
            target: {
                src: source,
                dest: 'build/shiroim.js'
            }
        },
        build: {//自定义插入合并
            target: {
                banner: banner,
                src: 'build/shiroim.js',
                info: ['<%= pkg.version %>', '<%= pkg.author %>', '<%= pkg.email %>'],
                dest: ['build/shiroim-<%= pkg.version %>.js']
            }
        },
        clean: {// 删除临时文件
            target: {
        ...
        jshint: { //语法检查
            ...
            target: 'build/shiroim-<%= pkg.version %>.js'
        },
        uglify: { //压缩代码
            options: {
                banner: banner
            ...
            target: {
                ...
                files: [{
                    'build/shiroim-<%= pkg.version %>.min.js': ['build/shiroim-<%= pkg.version %>.js']
                }]
            }
        },
        qunit: {//单元测试
